...
function ColHead({ title, sub }: { title: string; sub: string }) {
  return (
    <div className="pb-2.5 mb-1.5 border-b border-sage/10">
      <p className="font-semibold text-sage leading-none font-sans text-[0.68rem] uppercase tracking-[0.18em]">
        {title}
      </p>
      ...
      {isOpen && (
        ...
        >
          ...
          <motion.div variants={colV} className="flex flex-col gap-0.5 pr-4">
            <ColHead title={t.hq_title} sub="Ministry focus" />
            <Item href={`${base}/departments`} icon="📡" label={t.dept.mediaComm} />
            <Item href={`${base}/departments`} icon="📖" label={t.dept.trainingTeaching} />
            <Item href={`${base}/departments`} icon="🤝" label={t.dept.charityDevelopment} />
          ...
          <motion.div variants={colV} className="flex flex-col gap-0.5 px-4">
            <ColHead title={t.edu_title} sub="Education" />
            <Item href={`${base}/lms/login?program=diploma`} icon="📜" label={t.edu.diploma} />
            <Item href={`${base}/lms/login?program=degree`} icon="🎓" label={t.edu.degree} />
            <Item href={`${base}/lms/login?program=masters`} icon="🏛" label={t.edu.masters} />
            <Item href={`${base}/library`} icon="📚" label={t.edu.library} />
          </motion.div>

          <motion.div variants={colV} className="flex flex-col gap-0.5 pl-4">
            <ColHead title={t.integration_title} sub={t.integration_sub} />
            <Item href={`${base}/integration`} icon="🔗" label={t.int.library} />
            <Item href={`${base}/lms/login`} icon="💻" label={t.int.lms} />
            <Item href={`${base}/integration#graduation`} icon="🎓" label={t.int.graduation} />
            <div className="mt-3 p-2.5 rounded-lg bg-sage/[0.05] border border-sage/10">
              <p className="text-mist/42 leading-relaxed font-sans text-[0.64rem]">
                {t.structureDesc}
              </p>
            </div>
          </motion.div>
